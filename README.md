...
Supp Data 4 and 5, containing a clean version of source data, should be downloaded from paper and added to the Supp Data folder. Note the .RProj isn't strictly necessary but it avoids having to change any directory paths as its directory is automatically accessed by here::here() in R (should work even if you don't access the code from the .RProj file; just having it in directory is in theory enough).
