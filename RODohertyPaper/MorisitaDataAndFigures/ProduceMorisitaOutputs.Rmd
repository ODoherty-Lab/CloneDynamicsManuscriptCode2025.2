---
title: "ProduceMorisitaFiguresAndSuppData34"
author: "Sam Weissman"
output: html_document
---

Be sure to first run the SuppDataToRDSObjects.R code.
...
pVals6.1vs8.9 <- apply(outTable[,-ncol(outTable)], 2, function(vec) {
  quiet_barnard(vec[1], vec[2], totalProvirusesSequenced[1]-vec[1], totalProvirusesSequenced[2]-vec[2])*2 # Two-sided Barnard with Bonferroni correct for two comparisons between timepoints
})
pVals8.9vs9.9 <- apply(outTable[,-ncol(outTable)], 2, function(vec) {
  quiet_barnard(vec[2], vec[3], totalProvirusesSequenced[2]-vec[2], totalProvirusesSequenced[3]-vec[3])*2
})
...
(basePlot1 + ggtitle("Supp Figure Early Timepoints") + geom_point(size=12, alpha=0.65)) %>% 
...
(basePlot2 + ggtitle("Supp Figure Late Timepoints") + geom_point(size=12, alpha=0.65)) %>% 
...
# Part B
 # add trendlines of morisita as funciton of time elapsed for several participants, and add the points for EC4/EC5
(basePlot + ggtitle("Fig. 5b") + geom_point(size=10, alpha=0.4)) %>%
  addToGraph(simpleObject = provirusObjects$CT9, addPoints = FALSE, addLine = TRUE, lineCol = "#22c571dc", linetype = "dotted", lineSize = 4.5) %>%
...
(basePlot2 + ggtitle("Fig. 5c") + geom_point(size=10,alpha=0)) %>%
 ...
 addToGraph(simpleObject = correctedTCRObjects$EC4, distanceMat = -TCRDistMats$EC4, addPoints = TRUE, addLine = FALSE, pointCol = "#d1acfeff", psize = 8) %>%
  addToGraph(simpleObject = correctedTCRObjects$EC5, distanceMat = -TCRDistMats$EC5, addPoints = TRUE, addLine = FALSE, pointCol = "#d1acfeff", psize = 8)
...
Make and save Supp Data 3 and 4
```{r, eval = FALSE}

# Supp Data 3
# Uses resamples = 10000 in paper

correctedTCRSheets <- makeVennSheetsList(list(), correctedTCRObjects, "TCR", fullTCRTables = TCRFullTables, resamples = 50)

simpleObjectSheets <- makeVennSheetsList(list(), provirusObjects, "Proviruses", resamples = 50)
simpleObjectSheets <- c(simpleObjectSheets, correctedTCRSheets)
library(openxlsx)
write.xlsx(simpleObjectSheets, file=paste0(base, "/MorisitaDataAndFigures/SuppData/SupplementaryData3.xlsx"),rowNames=F, colNames=FALSE) # note due to rounding off each timepoints, the timepoint differences may be 0.1 off from rounded true differences recorded in true Supp Data 3.
...
write.xlsx(together, file=paste0(base, "/MorisitaDataAndFigures/SuppData/SupplementaryData4.xlsx"), rowNames = F, colNames = F)
