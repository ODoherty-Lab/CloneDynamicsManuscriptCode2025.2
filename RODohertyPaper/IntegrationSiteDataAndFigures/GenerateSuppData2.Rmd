...
Merge new and old Bushman data, make supp data 1.
```{r}
...
CT1CT2MergedTables[[1]] <- mergeTables(BushmanDataFirstT2T[[1]], BushmanDataSecond[[1]]) # column 15 is new at 10.4 years on ART. column 16 is same time as 13. Column 17 similarly goes with 14
...
CT1CT2MergedTables[[2]] <- mergeTables(BushmanDataFirstT2T[[2]], BushmanDataSecond[[2]]) # column 12 is new at 13.20 years on ART. Column 13 is new at 14.24 years on ART.
...
toOutput <- CT1CT2MergedTables # tidy up for supp data 1
for (i in 1:length(toOutput)) {
  toOutput[[i]][,2] = gsub(" .*","",toOutput[[i]][,2]) # tidy up gene names
  toOutput[[i]][which(toOutput[[i]][,5] == "exonic"),5] = "notIntronic"
...
library(openxlsx)
#write.xlsx(toOutput, file=paste0(base, "/IntegrationSiteDataAndFigures/SupplementaryData1Pre.xlsx"),rowNames=F, colNames=FALSE) # in real supp data 1 we rearrange columns a bit.

# make the formatting of it in Supp Data 2.
...
#### format into supp data 2. Remove non-introns and then redo merging exactly as above except with keepIntronic:
...
OurCTs <- processCTMats(CT1CT2MergedTablesIntronic) # make supp data 2 format
...
# this function specifies it to the Guo data and outputs table formatted like in supp data 2. 
...
allCombined <- mergeParticipantTables(list(a=OurCTs, b=HughesTogetherTable)) # note not quite identical to supp data 2 as inevitably some aliases changed names but identical if you re-alias supp data 2.

allCombined$Orientation <- ifelse(allCombined$Orientation == 1, "Sense", "Antisense")

```

Get lists of growth-related genes and highly-expressed genes as in supp data 2 sheets 2/3
```{r}
...
# can use these to generate figures, are similar in format to supp data 2 and same in content.
