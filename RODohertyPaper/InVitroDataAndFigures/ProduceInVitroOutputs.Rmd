...
names(tablesToWriteList) <- c("Pre read counts", "Post read counts", "HIV Read Counts") # to get read counts without dividing by the exon length, as in the real supp data 6, remove division by sum(as.numeric(featureCountsPart[,6])) in organizeHeatmapData.R function getCombinedExpression. Then re-run former code block.
openxlsx::write.xlsx(tablesToWriteList, paste0(pathToSuppData, "/preSuppData6PerBP.xlsx"),rowNames=TRUE)
