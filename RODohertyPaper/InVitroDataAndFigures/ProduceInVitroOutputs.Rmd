...
heatmapPreTPM <- apply(heatmapPre[-1, -1], 2, function(x) {1E6*as.numeric(x)/MappedTranscriptLevels})
heatmapPostTPM <- apply(heatmapPost[-1, -1], 2, function(x) {1E6*as.numeric(x)/MappedTranscriptLevels})
heatmapHIVTPM <- apply(heatmapHIV[-1, -1], 2, function(x) {1E6*as.numeric(x)/MappedTranscriptLevels})



heatmapPreLogTPM <- apply(heatmapPre[-1, -1], 2, function(x) {log(1E6*as.numeric(x)/MappedTranscriptLevels)})
heatmapPostLogTPM <- apply(heatmapPost[-1, -1], 2, function(x) {log(1E6*as.numeric(x)/MappedTranscriptLevels)})
heatmapHIVLogTPM <- apply(heatmapHIV[-1, -1], 2, function(x) {log(1E6*as.numeric(x)/MappedTranscriptLevels)})
...
heatmapHIVLogTPMStandardized <- standardizeExpression(heatmapHIVLogTPM, heatmapHIVLogTPMAveraged)


saveHeatmaps(heatmapPreLogTPMStandardized, heatmapPostLogTPMStandardized, heatmapHIVLogTPMStandardized,
             paste0(base, "/InVitroDataAndFigures/Figures/"), "Clone Expression, Normalized by TPM and Gene Expression")


```

Make the formal supplementary data
```{r}
...
write.xlsx(tablesToWriteList, paste0(pathToSuppData, "/preSuppData6.xlsx"),rowNames=TRUE)
