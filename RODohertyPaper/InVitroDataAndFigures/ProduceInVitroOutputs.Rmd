...
# be sure to unzip concatenatedExonFeatureCounts.csv.zip in supp data rna seq files folder.
...
siteData <- readxl::read_xlsx(paste0(pathToSuppData, "Supplementary Data 7-T Cell Clone Sites TPM.xlsx"))
...
concatenatedExonFeatureCounts <- read.csv(paste0(pathToSuppData, "/RNASeqProcessed/concatenatedExonFeatureCounts.csv"))[,-1] # be sure to UNZIP
...
# for genes where HIV was before last exon, the code hits an edge case and also includes the index of the exon after the end of the gene in addition to the end of the gene, so this just fixes those. Can verify by checking GeneIndicesLastExon$MYB and GeneIndicesLastExon$C5orf24 before these updates.
GeneIndicesLastExon$MYB = 323360
GeneIndicesLastExon$C5orf24 = 273393


```


Produces data for the heatmap, which is also Supp Data 8.
...
names(tablesToWriteList) <- c("Pre read counts", "Post read counts", "HIV Read Counts") # to get read counts without dividing by the exon length, as in the real supp data 8, remove division by sum(as.numeric(featureCountsPart[,6])) in organizeHeatmapData.R function getCombinedExpression. Then re-run former code block.
openxlsx::write.xlsx(tablesToWriteList, paste0(pathToSuppData, "/preSuppData8PerBP.xlsx"),rowNames=TRUE)
